import mockDataV7 from './trio_mock_data_v7.json';

export const cashRecords = mockDataV7.cash;
export const capexRecords = mockDataV7.capex;
export const equityRecords = mockDataV7.equity;
...
export const getMonthlyBurn = (
  ...
  const cashData = cashRecords.filter(r =>
    r.scenario === scenario &&
    r.date >= startDate &&
    r.date <= endDate
  );
...
export const getPayables = (
  ...
  // Filter COGS + OPEX + CapEx where cash_date > currentDate
  const cogsOutstanding = mockDataV7.cogs.filter(t => 
    t.scenario === scenario && 
    t.cash_date > currentDate &&
    (bu ? t.bu === bu : true)
  );
  
  const opexOutstanding = mockDataV7.opex.filter(t => 
    t.scenario === scenario && 
    t.cash_date > currentDate &&
    (bu ? t.bu === bu : true)
  );
...
export const getReceivables = (
  ...
  const outstanding = mockDataV7.revenues.filter(t =>
    t.scenario === scenario &&
    t.cash_date > currentDate &&
    (bu ? t.bu === bu : true)
  );
