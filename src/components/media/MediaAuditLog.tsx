// Audit trail panel — who/when/field/old/new for every Media CMS edit.
// Reads site_media_audit directly (migration 057). Every row here was
// written atomically by the same DB function that made the edit
// (media_update_field / media_create_asset / media_delete_asset) — there
// is no code path that can write a media change without one.
import { History } from "lucide-react";
import { Card } from "@/components/ui/card";
import { Table, TableHeader, TableBody, TableRow, TableHead, TableCell } from "@/components/ui/table";
import { useMediaAudit } from "@/data/mediaLive";

export const MediaAuditLog = () => {
  const { data, isLoading, isError } = useMediaAudit(100);
            ...
            <TableBody>
              {data.map((row) => (
                <TableRow key={row.audit_id}>
                  ...
                  <TableCell className="font-mono text-xs">{row.media_key}</TableCell>
